...
/*
 *	 1.进程组:每一个进程都属于一个"进程组"，当一个子进程被创建的时候，它默认是其父进程所在组
 *	的成员。一个进程的组ID(PGID)等于这个组的第一个成员(也称为进程组长)。
 *		查看指令: ps -ejH
 *
 *	 2.会话:内核将几个进程组并为一个"会话"。会话ID就是通过setsid()启动这个会话的进程的PID
 *  (也就是这个会话的第一个进程，通常是用户的shell)，这个进程也称为"会话头"，它随后产生的所
 *  有子孙进程都默认在这个会话里。
 *
 ...
 *	通常会暂时停止此后台作业。当有输出到终端时，会直接向终端输出。
 *
 *	 5.孤儿进程组:该进程组中每一个进程的父进程都属于另一个session，孤儿进程组是后台进程组，当
 *	去读控制终端时，终端会向后台作业发送一个特定的信号SIGTTIN(暂停)，向每一个暂停状态的进程发
 *	送挂断信号(SIGHUP终止进程)，接着又向暂停进程发送继续运行信号(SIGCONT)。当有输出到终端时，
 *	会直接向终端输出。
 * */
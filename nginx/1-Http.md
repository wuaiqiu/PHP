...
HTTP/1.0:第二个版本的HTTP协议，它增加请求头部与响应头头部，不只限于纯文本传输，支持GET、HEAD、POST方法，支持长连接即在一次TCP连接中进行多次HTTP连接传输(但默认还是使用短连接)，支持缓存机制以及身份认证。


HTTP/1.1:第三个版本的HTTP协议，是目前使用最广泛的协议版本。默认使用长连接(在HTTP/1.0中使用长连接需要添加请求头 Connection: Keep-Alive)，使用chunked编码传输(将未知长度的实体进行分块传送并逐块标明长度,直到长度为0块表示传输结束,需要注明 Transfer-Encoding: chunked)，支持字节范围请求(即传送内容的一部分。比方说，当客户端已经有内容的一部分，为了节省带宽，可以只向服务器请求一部分。该功能通过在请求消息中引入了range头域来实现，它允许只请求资源的某个部分。在响应消息中Content-Range头域声明了返回的这部分对象的偏移值和长度。如果服务器相应地返回了对象所请求范围的内容，则响应码206(Partial Content))，HTTP/1.1增加了OPTIONS,PUT, DELETE, TRACE, CONNECT方法。HTTP/1.1在HTTP/1.0缓存机制的基础上加入了E-tags头。
...
Pragma: no-cache|与具体指令相关，no-cache指令与Cache-Control含义相同
If-Modified-Since: Dec, 26 Dec 2015 17:30:00 GMT|允许在对应的资源未被修改的情况下返回304(Not Modified)
If-Match: "9jd00cdj34pss9ejqiw39d82f20d0ikd"|设置客户端的ETag,当时客户端ETag和服务器生成的ETag一致才执行，适用于更新自从上次更新之后没有改变的资源
If-None-Match: "9jd00cdj34pss9ejqiw39d82f20d0ikd"|设置客户端ETag，如果和服务端接受请求生成的ETage相同，允许服务端返回304(Not Modified)
If-Range: "9jd00cdj34pss9ejqiw39d82f20d0ikd"|设置客户端ETag，如果和服务端接受请求生成的ETage相同，返回缺失的实体部分；否则返回整个新的实体
...
Host: www.itbilu.com|表示服务器的域名以及服务器所监听的端口号
From: user@example.com|设置发送请求的用户的email地址
User-Agent: Mozilla|浏览器的身份标识字符串
Origin: http://www.itbilu.com|发起一个针对跨域资源共享的请求
Referer: http://itbilu.com/nodejs|表示浏览器所访问的前一个页面
Upgrade: HTTP/2.0, SHTTP/1.3, IRC/6.9, RTA/x11|要求服务器升级到一个高版本协议
...
Max-Forwards: 10|限制该消息可被代理及网关转发的次数
X-Forwarded-For: 129.78.138.66, 129.78.64.103|用来标识客户端通过HTTP代理或者负载均衡器连接的web服务器的原始IP地址
Via: 1.0 fred, 1.1 itbilu.com.com (Apache/1.1)|告诉服务器，这个请求是由哪些代理发出的
...
Cache-Control: max-age=3600|通知客户端缓存这个对象及缓存有效时间。其单位为秒
Expires: Thu, 01 Dec 1994 16:00:00 GMT|指定一个日期/时间，超过该时间则认为此回应已经过期
Last-Modified: Dec, 26 Dec 2015 17:30:00 GMT|所请求的对象的最后修改日期
Pragma: no-cache|与具体指令相关，no-cache指令与Cache-Control含义相同
Content-Type: text/html; charset=utf-8|当前内容的MIME类型
Content-Encoding: gzip|响应资源所使用的编码类型
Content-Language: zh-cn|响就内容所使用的语言
...
Content-Range: bytes 21010-47021|如果是响应部分消息，表示属于完整消息的哪个部分
Date: Tue, 15 Nov 1994 08:12:31 GMT|此条消息被发送时的日期和时间(以RFC 7231中定义的"HTTP日期"格式来表示)
ETag: "737060cd8c284d8af7ad3082f209582d"|对于某个资源的某个特定版本的一个标识符，通常是一个消息散列
Location: http://www.itbilu.com/nodejs|	用于在进行重定向
Refresh: 5; url=http://itbilu.com|用于重定向，默认会在5秒后刷新重定向
Server: nginx/1.6.3|服务器的名称
X-Powered-By: PHP/5.4.0|指定支持web应用的技术
Connection: close|服务端想要优先使用的连接类型
Status: 200 OK|	通用网关接口的响应头字段，用来说明当前HTTP连接的响应状态
Set-Cookie: UserID=itbilu; Max-Age=3600; Version=1|设置HTTP cookie
Transfer-Encoding: chunked|用表示实体传输给用户的编码形式。包括：chunked、compress、 deflate、gzip、identity
Upgrade: HTTP/2.0, SHTTP/1.3, IRC/6.9, RTA/x11|要求客户端升级到另一个高版本协议
Via: 1.0 fred, 1.1 itbilu.com (nginx/1.6.3)|告知代理服务器的客户端，当前响应是通过什么途径发送的
